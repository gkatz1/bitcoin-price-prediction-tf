...
def preprocess_data(data_params):
    ...
    # print(daily_price)
    # daily_price = difference(daily_price, look_back)
    # print("*" * 50)
    ...
    # print(train_set)
    train_set = difference(train_set, look_back)
    # print(train_set)
    ...
    ## test_set = scaler.transform(test_set)   # don't want to scale test data's labels
    
    # print(train_set.shape)
    # train_set = difference(train_set)
    # test_set = difference(test_set) 
    ...
    X_test = scaler.fit_transform(X_test)  # now y_test are the raw values
    ...
    with tf.Session() as sess:
        ...
        for epoch in tqdm.tqdm(range(1)):
            ...
            for i, data in tqdm.tqdm(enumerate(val_loader, 0), total=num_validation_batches): 
                ...
                y_inverse = y_inverse.squeeze(axis=0)
                rmse = sqrt(mean_squared_error(prediction_inverse, y_inverse))
                print("inversed: prediction, ground truth")
                print(prediction_inverse)
                print(y_inverse)
                print("rmse: {}".format(rmse))
                print("\n")

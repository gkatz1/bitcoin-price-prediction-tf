...
def difference(data, look_back=1):
    diff = list()
    for i in range(look_back, len(data)):
        value = data[i] - data[i - look_back]
        diff.append(value)
    return np.array(diff)


def inverse_difference(history, yhat, look_back=1):
    return yhat + history[-look_back]
...
def invert_scale(scaler, vals, value_to_invert):
    tmp_row = [v for v in vals] + [value_to_invert]
    arr = np.array(tmp_row)
...
def train(train_params):
    ...
    rnn_cell = tf.nn.rnn_cell.MultiRNNCell([tf.nn.rnn_cell.LSTMCell(n_hidden,

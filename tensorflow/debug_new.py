...
def train(train_params):
    ...
    init_state_0_attention_0 = tf.placeholder(tf.float32, [None, attn_len ** 2])
    init_state_0_attention_1 = tf.placeholder(tf.float32, [None, attn_len ** 3])
    ...
    _cur_state_0_attention_0 = np.zeros((batch_size, attn_len ** 2), dtype=np.float32)
    _cur_state_0_attention_1 = np.zeros((batch_size, attn_len ** 3), dtype=np.float32)
    _current_state = tuple((tuple((_cur_state_0_c, _cur_state_0_h)), _cur_state_0_attention_0, 
        _cur_state_0_attention_1))
            ...
            for i, data in tqdm.tqdm(enumerate(train_loader, 0), total=num_training_batches):
                ...
                _pred, _loss, _, _summary, _current_state = sess.run([
                    ...
                    feed_dict = {
                        ...
                        init_state_0_attention_0: _current_state[1],
                        init_state_0_attention_1: _current_state[2]
                    }
                )
